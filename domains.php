...
//Check if we're updating something
	...
	if(isset($_REQUEST['formType'])){
		if(isset($_POST['formType']) && $_POST['formType'] == "add"){
			$sql = "INSERT INTO `websites` (domain, lastSeen, lastError, lastStatus, emailSent) VALUES ('" . safesql($_POST['domain']) . "', 'Not Checked Yet', 'Not Checked Yet', 'Not Checked Yet', false)";
			$stm = $con->prepare($sql);
			$stm->execute();
			if($stm){
